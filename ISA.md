...
### Dumbbell workouts + upper-body stretches (2026-07-29, Austin's repeated ask, KOT + Huberman grounded)
- [x] ISC-473: DUMBBELL_WORKOUTS constant in app.js holds 3 routines, every movement dumbbell or bodyweight only, probe: Read app.js
- [x] ISC-474: every exercise entry carries name, sets x reps, and a form cue, probe: Read app.js
- [x] ISC-475: set/rep/rest schemes follow fetched Huberman/Galpin guidance (3x5 strength protocol; 8 to 15 rep hypertrophy range), source URLs cited in an HTML comment, probe: grep index.html
- [x] ISC-476: at least 3 KOT signature movements included (ATG split squat, DB RDL, external rotation, cross bench pullover, trap 3 raise), probe: grep app.js
- [x] ISC-477: Dumbbell strength section renders on the Train tab, one card per routine, probe: Interceptor screenshot
- [x] ISC-478: each workout card has a Log button that POSTs a strength activity with the routine's minutes and name, probe: browser click + DB SELECT
- [x] ISC-479: a logged dumbbell workout never increments the G1 qualifying count, probe: bun test
- [x] ISC-480: all new copy is routed through escapeHtml at render, probe: Read render function
- [x] ISC-481: STRETCH_PLAN extended with 5 upper-body items, each with slug, name, dose, target, cue, probe: Read app.js
- [x] ISC-482: upper-body items include the KOT hanging staple plus pec, t-spine, rear shoulder, wrist coverage, probe: Read app.js
- [x] ISC-483: stretch doses follow the fetched Huberman protocol (30s static holds, 2 to 4 sets, low intensity), probe: Read doses
- [x] ISC-484: each new stretch has a hand-authored SVG at public/img/stretch/{slug}.svg matching the existing visual style, probe: ls
- [x] ISC-485: every STRETCH_PLAN slug (all 13) serves over the existing regex route with image/svg+xml, probe: bun test parses slugs from app.js
- [x] ISC-486: stretch intro and footnote updated to cover upper body and cite Huberman alongside Ben Patrick, probe: Read index.html
- [x] ISC-487: full stretch list including the 5 new items renders in a real browser, probe: Interceptor screenshot
- [x] ISC-488: full suite green and tsc clean, probe: bun test + tsc
- [x] ISC-489: Anti: zero em/en dashes anywhere in the new content including comments, and a guard test scans public/app.js + index.html user-facing strings, probe: bun test + grep
- [x] ISC-490: Anti: no new runtime dependencies and no server code change required, probe: git diff package.json + src/server.ts empty
...
| 473-490 | feature | data/SVG probes, slug-parsing serve test, dash guard, bun test, Interceptor walkthrough | suite green, 13 SVGs serve, flows render | Bash/Interceptor |
...
| dumbbell-workouts | 3 dumbbell-only routines on the Train tab (KOT movements, Huberman/Galpin schemes) with one-tap strength logging | ISC-473..480 | web-ui, api | false |
| upper-body-stretches | 5 upper-body additions to STRETCH_PLAN with SVGs, Huberman-protocol doses | ISC-481..487 | stretch-plan | false |

## Decisions

- 2026-07-29 — **Dumbbell workouts + upper-body stretches (ISC-473..490), Austin: "Build a workout routine with just dumbells to the cadence app, also add some more streches of the upper body to the strecthing routine. Refernce kneesovertoesguys and Andrew Huberman".** This is his SECOND ask: the 2026-07-29 morning session (failure capture 073616) received this exact request and went off-topic onto Suretas deploy work, a 3/10 signal. Priority this run is complete, on-target delivery. Tier E2 (classifier). Content grounded by live fetch, not recall: ATG standards list (a1athlete.com/atg-standards: external rotation 5 to 10% BW x10, cross bench pullover 25% BW x10, trap 3 raise 10% BW, DB split squat 25% BW per hand, DB RDL 25% BW per hand x20), Huberman 3x5 strength protocol (LinkedIn post) + 8 to 15 rep hypertrophy range (readandrewhuberman.com), Huberman flexibility protocol (hubermanlab.com Essentials episode: 30s static holds, 2 to 4 sets, 5 to 6 days/week, 30 to 40% intensity, warm first). Design mirrors shipped patterns exactly: DUMBBELL_WORKOUTS constant like SWIM_SETS with per-card log button posting sport=strength (never G1), STRETCH_PLAN appended in place (flat list, lower body then upper), new SVGs ride the existing regex img route so zero server change. Delegation floor relaxed (show-my-math): single-author two-file frontend content work; Engineer agents have stalled mid-build on Cadence four recorded times, and a fifth stall on an already-repeated request is the worst outcome available. New guard test parses STRETCH_PLAN slugs out of app.js and asserts every SVG serves, converting the data-to-asset link from convention to enforced property. (ISC-399..420), Austin: "add a golf section... I have garmin golf to keep track".** Science probe first: garmin-connect-sdk exposes ZERO golf/scorecard endpoints, so scorecards are impossible via this SDK; design is rounds-from-sync (Garmin golf typeKey activities, currently mapped to 'other') plus a user-entered golf_score, with holes surfacing for free when Garmin's lap detail carries them. First-class sport='golf' chosen over a raw_type-driven read path (label/filter cleanliness; advisor concurred). The CHECK-constraint table rebuild is the risky piece; advisor caught a FATAL placement bug in my draft: PRAGMA foreign_keys is a no-op inside a transaction, so the rebuild would have dropped activities with FKs live and cascade-deleted activity_details. Adopted: FK pragma outside txn with finally-restore + foreign_key_check, DDL copied from sqlite_master with only the CHECK fragment modified (never hand-retyped), ALL sqlite_master dependents (indexes/triggers/views) enumerated and re-executed, before/after COUNT assertions on activities AND activity_details as the oracle, explicit id preservation. raw_type remap ('other'→'golf') runs as a separately guarded step so schema rollback and data mutation stay disentangled. 9th tab is safe now because the layout run made the phone nav a scrollable rail. Wave 3 (pacing/YoY/power curve UI) finishes first so the tree never holds two half-features. Primary builds (show-my-math: two Engineer stalls in two waves, full context loaded, both slates are small-to-medium patterned work).
...
### Round-2 waves + golf verification (2026-07-23)
- Waves 1-3 + golf: 283 tests pass, tsc clean, zero new deps, commits 41b3ac6 / 87739f5 / wave-3 / ba926cb (one per wave, per the advisor discipline).
- Golf rebuild migration: proven THREE ways: unit suite on a synthetic pre-golf DB (rows/details/ids/indexes/cascade preserved, double-boot no-op, disc_golf guard), a LIVE rebuild on the scratch instance whose DB genuinely predated golf (20 activities + 1 detail row survived, zero foreign_key_check violations), and the fresh-DB path (golf present without rebuild).
- golf_score provenance: sync structurally never writes the column (rpe pattern); explicit passing test edit-then-resync keeps 89.
- Browser-verified on seeded scratch via Interceptor: pacing line with sport-mix suggestion ("One more swim closes it") + at-risk flag, YoY chips with real deltas, power curve SVG (612/388/265/217 W), golf tab (chips 3/89/92/Jul-19, canonical rows, inline score edit 101 DB-round-trip, hash click-in).
- State label per advisor: build-complete, live-integration-unverified. Live-shape deferrals: FOLLOWUP-cadence-detail-live-shape (Garmin getSplits/getDetails), ZP getCriticalPowerProfile shape, both behind defensive parsers with degrade-to-null and one-time key-set logs. Deploy gated on Austin (FOLLOWUP-cadence-round2-deploy: deploy, then real-data walkthrough, then mark ROADMAP round-2 shipped).

### Dumbbell workouts + upper-body stretches verification (2026-07-29)
- ISC-473..476: Read app.js — 3 routines, 15 exercises, all dumbbell/bodyweight; 5 KOT movements present (test asserts ATG Split Squat, External Rotation, Cross-Bench Pullover, Trap 3 Raise, Dumbbell RDL); source URLs cited in index.html comment (grep confirmed).
- ISC-477/487: Interceptor real-browser screenshots on scratch instance (port 4199, throwaway DB) — Dumbbell strength section renders 3 cards with scheme lines and log buttons; stretch list renders all 13 items; every one of the 13 img elements confirmed loaded via eval (complete && naturalWidth>0, all true).
- ISC-478: clicked "Log this workout" on Full Body Strength A in the real browser; DB SELECT returned {sport:"strength", title:"Dumbbells: Full Body Strength A", duration_s:2100}.
- ISC-479: test/dumbbell.test.ts — strength workout does not increment sessions or hours_g1 (suite green).
- ISC-480: render functions route every field through escapeHtml (Read of renderDumbbellWorkouts + stretch render).
- ISC-481..484: Read app.js STRETCH_PLAN — 5 upper items (dead-hang, doorway-pec, t-spine-reach, cross-body-shoulder, wrist-stretch) with 30-45s doses per the fetched Huberman protocol; 5 hand-authored SVGs in existing style, curl on scratch returned 200 image/svg+xml.
- ISC-485: dumbbell.test.ts parses STRETCH_PLAN slugs out of app.js and serves each through fetchHandler — 13/13 green, converting data-to-asset linkage into an enforced property.
- ISC-486: intro now reads "lower body then upper body. ~20 minutes"; footnote cites ATG standards + Huberman flexibility protocol (rendered text captured in browser).
- ISC-488: bun test 329 pass / 0 fail (18 new); bunx tsc --noEmit exit 0.
- ISC-489: dash-guard test scans STRETCH_PLAN + DUMBBELL_WORKOUTS blocks and the Train-tab HTML section — zero em/en dashes; SVG grep also zero.
- ISC-490: git diff package.json + src/server.ts empty — zero new deps, zero server changes (new SVGs ride the existing regex route).
- Screenshot-artifact note: the Interceptor full-page capture showed exercise names overlapping cues; reproduced against the LIVE DOM at 1368px and forced 340px card width — zero overlap, all names single-line (getBoundingClientRect measurements). Capture-renderer font-metric artifact, not a layout bug.
