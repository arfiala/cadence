...
- [DEFERRED-VERIFY] ISC-252: the live Garmin sleep payload's exact field names/units are confirmed on the first real prod sleep sync; until then the adapter reads defensively and logs the raw key set when a non-empty payload yields all-null nights, so a shape mismatch surfaces loudly. Follow-up: FOLLOWUP-cadence-sleep-live-shape
